...
void TCPSender::fill_window() {
   ...
   if (_receiver_window_size) {
	   ...
	   // since fin will occupy 1 byte space, so must make sure receiver has one extra free space for fin
...
void TCPSender::_send_segment(TCPSegment &seg) {
    ...
    _bytes_in_flight += seg.length_in_sequence_space();
    if (_syn_sent) _receiver_free_space -= seg.length_in_sequence_space();
    _segments_out.push(seg);
